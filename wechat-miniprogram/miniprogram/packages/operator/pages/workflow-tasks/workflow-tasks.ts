...
Page<Data>({
  ...
  onLoad() {
    const accessReady = ensureOperatorPageAccess("/packages/operator/pages/workflow-tasks/workflow-tasks");
    this.setData({ accessReady });
    if (accessReady) void this.loadTasks(true);
  },
  onPullDownRefresh() {
    if (this.data.accessReady) void this.loadTasks(true, true);
  ...
  async loadTasks(reset = true, fromPullDown = false) {
    ...
    try {
      const page = await operatorWorkflowApi.tasks({ cursor: reset ? undefined : this.data.nextCursor || undefined, limit: 100 });
      if (sequence !== this.requestSequence) return;
      const incoming = page.items.map((item) => workflowTaskView(item, "operator"));
      ...
      if (sequence !== this.requestSequence) return;
      const message = error instanceof Error ? error.message : "履约待办暂时无法读取";
  ...
  openTask(event) {
    ...
    if (!item) return;
    const target = resolveWorkflowActionTarget(item.actionCode, item.actionParams, "operator");
    if (target) wx.navigateTo({ url: target });
    else wx.showToast({ title: "该待办无需页面操作", icon: "none" });
  },
