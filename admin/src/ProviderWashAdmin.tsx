...
export function ProviderWashOrdersPage({ canRedeem, onError }: { canRedeem: boolean; onError: (message: string) => void }) {
  ...
  const load = useCallback(async () => {
    ...
    } catch (reason) { onError(operatorErrorMessage(reason, "洗车订单读取失败，请稍后重试")); }
  ...
  const openOrder = async (order: ProviderOrder) => {
    ...
    catch (reason) { onError(operatorErrorMessage(reason, "洗车订单详情读取失败，请稍后重试")); }
  };
  const redeemCode = async (verificationCode: string, orderId?: string, note?: string) => {
    ...
    } catch (reason) { onError(operatorErrorMessage(reason, "订单核销失败，请核对后重试")); }
  ...
  return <>
    <section className="provider-redeem-bar">
      <div><span><Key weight="duotone" /></span><div><small>人工核销</small><h2>快速核销</h2><p>请当面核对车主出示的六位码；系统不会提前显示或返回核销码。</p></div></div>
    ...
    <section className="content-card provider-order-card"><div className="filters provider-order-filters"><span><SlidersHorizontal />筛选</span><input aria-label="服务商洗车订单号" value={filters.orderNumber} onChange={(event) => updateFilter("orderNumber", event.target.value)} placeholder="订单号" /><input aria-label="服务商洗车预约日期" type="date" value={filters.date} onChange={(event) => updateFilter("date", event.target.value)} /><select aria-label="服务商洗车服务方式" value={filters.serviceMode} onChange={(event) => updateFilter("serviceMode", event.target.value)}><option value="">全部服务方式</option><option value="self_drive">自驾到店</option><option value="valet">往返代驾</option></select><select aria-label="服务商洗车订单状态" value={filters.status} onChange={(event) => updateFilter("status", event.target.value)}><option value="">全部订单状态</option>{Object.entries(orderStatusLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><button onClick={() => { setPage(1); setFilters({ orderNumber: "", date: "", serviceMode: "", status: "" }); }}>清空</button></div>
      <div className="table-wrap"><table><thead><tr><th>订单与时段</th><th>车辆</th><th>套餐</th><th>服务方式</th><th>金额</th><th>状态</th><th /></tr></thead><tbody>{orders.map((order) => <tr key={order.id} onClick={() => void openOrder(order)}><td><strong>{order.orderNumber}</strong><small>{order.appointmentDate} {order.startTime}–{order.endTime}</small></td><td><strong>{order.vehiclePlate || order.plateNumber || "待登记"}</strong><small>{vehicleLabels[order.vehicleType || ""] || "车型待确认"}</small></td><td><strong>{order.packageName || order.package?.name || "洗车服务"}</strong></td><td><span className={`mode-pill ${order.serviceMode}`}>{order.serviceMode === "valet" ? "往返代驾" : "自驾到店"}</span></td><td><strong>¥{money(orderAmount(order))}</strong></td><td><span className={`status-pill wash-status-${order.status}`}>{orderStatusLabels[order.status] || "状态待核对"}</span></td><td><CaretRight /></td></tr>)}</tbody></table>{loading ? <div className="table-loading">正在读取本店订单…</div> : !orders.length ? <div className="empty-table">没有符合条件的本店订单</div> : null}</div>
...
function ProviderOrderDrawer({ order, canRedeem, saving, close, redeem }: { order: ProviderOrder; canRedeem: boolean; saving: boolean; close: () => void; redeem: (code: string, note: string) => Promise<void> | void }) {
  ...
  return <div className="drawer-layer" onMouseDown={(event) => event.target === event.currentTarget && close()}><aside className="detail-drawer provider-order-drawer" aria-label="服务商洗车订单详情"><header><div><small>门店订单</small><h2>{order.vehiclePlate || order.plateNumber || "待登记车辆"}</h2><p>{order.orderNumber}</p></div><button aria-label="关闭服务商洗车订单详情" onClick={close}><X /></button></header><div className="drawer-scroll">
    <section className="drawer-status"><span className={`status-pill wash-status-${order.status}`}>{orderStatusLabels[order.status] || "状态待核对"}</span><strong>{order.appointmentDate} · {order.startTime}–{order.endTime}</strong><small>{order.serviceMode === "valet" ? "往返代驾" : "自驾到店"} · 当前登录门店</small></section>
    <section className="detail-section"><h3>服务信息</h3><dl><div><dt>车辆</dt><dd>{order.vehiclePlate || order.plateNumber || "—"} · {vehicleLabels[order.vehicleType || ""] || "车型待确认"}</dd></div><div><dt>套餐</dt><dd>{order.packageName || order.package?.name || "洗车服务"}</dd></div><div><dt>服务方式</dt><dd>{order.serviceMode === "valet" ? "上门代驾取送（往返）" : "车主自驾到店"}</dd></div><div><dt>订单金额</dt><dd>¥{money(orderAmount(order))}</dd></div><div><dt>订单状态</dt><dd>{orderStatusLabels[order.status] || "状态待核对"}</dd></div></dl></section>
...
export function ProviderWashStorePage({ subjectId, onError }: { subjectId: string; onError: (message: string) => void }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      apply(next);
    } catch (reason) { onError(operatorErrorMessage(reason, "本店资料读取失败，请稍后重试")); }
    finally { setLoading(false); }
  }, [onError, subjectId]);
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        ...
        if ((reason as Error).name !== "AbortError") {
          ...
          setLocationMessage(`${operatorErrorMessage(reason, "地址联想暂不可用，请稍后重试")}；当前可信位置未改变`);
        }
      }
  ...
  const resolveMapPin = useCallback(async (latitude: number, longitude: number) => {
    ...
    try {
      ...
      if ((reason as Error).name !== "AbortError" && requestSequence === mapRequestSequence.current) {
        setMapError(`${operatorErrorMessage(reason, "地图落点核验失败，请稍后重试")}；当前可信位置未改变`);
      }
    } finally {
  ...
  const save = async (event: FormEvent) => {
    ...
    } catch (reason) { onError(operatorErrorMessage(reason, "门店资料保存失败，请稍后重试")); }
  ...
  const uploadImages = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (invalid) return showImageError("仅支持常用图片格式，且单张不能超过 10 兆字节");
    ...
    } catch (reason) { showImageError(operatorErrorMessage(reason, "门店图片上传失败，请稍后重试")); }
  ...
  const saveImageOrder = async (images: ProviderStoreImage[], message: string) => {
    ...
    } catch (reason) { onError(operatorErrorMessage(reason, "门店图片设置保存失败，请稍后重试")); }
    finally { setImageBusy(""); }
  };
  ...
  return <form className="content-card provider-store-editor" onSubmit={save}><header><div><small>本店资料</small><h2>{draft.name}</h2><p>资料保存后直接生效；平台排序、演示评分和账号绑定不在本页开放。</p></div><label className="switch"><input type="checkbox" checked={Boolean(draft.isActive && draft.isOpen)} onChange={(event) => { update("isActive", event.target.checked); update("isOpen", event.target.checked); }} /><span />{draft.isActive && draft.isOpen ? "开放预约" : "暂停预约"}</label></header>
    ...
    <section className="provider-store-section">
      <div className="provider-section-title provider-location-title">
        ...
        <div><small>已核验位置</small><h3>门店地址与定位</h3></div>
