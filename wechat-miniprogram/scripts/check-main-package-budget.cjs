...
const bundledOwnerVehicleImages = entries.filter(({ relativePath }) =>
  /^assets\/vehicles\/vehicle-[^/]+\.(?:png|jpe?g|webp|gif|svg)$/iu.test(relativePath),
);
if (bundledOwnerVehicleImages.length) {
  errors.push(`车型展示图片必须由 API/服务器加载，不得打进小程序包：${bundledOwnerVehicleImages.map(({ relativePath }) => relativePath).join("、")}`);
}
