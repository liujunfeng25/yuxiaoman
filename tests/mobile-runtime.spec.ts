...
async function drag(page: Page, locator: Locator, deltaX: number, deltaY: number, steps = 8) {
  ...
  await dragFromPoint(page, box.x + box.width / 2, box.y + box.height / 2, deltaX, deltaY, steps);
}
...
test("BottomSheet remains mounted while its default exit animation plays", async ({ page }) => {
  ...
  // Chrome 149 can report the scaled phone root as the hit-test owner even
  // though the phone-scoped overlay is visible. Force targets the verified
  // overlay element and keeps this regression focused on exit mounting.
...
test("keyboard and its attached footer dismiss on the same transition", async ({ page }) => {
  ...
  // Start on the footer chrome rather than its input. Interactive controls
  // intentionally opt out of the runtime's drag-to-dismiss gesture.
  await dragFromPoint(page, footerBox.x + 8, footerBox.y + 8, 0, 120, 5);
