...
export function VehicleCheckupReportPanel({
  ...
  return <section className="checkup-report" aria-label="车辆体检报告">
    ...
    <section className="checkup-evidence-summary" aria-label="报告材料摘要">
      <div className={hasSafetyInspectionReport ? "complete" : "incomplete"}><small>检测证明材料</small><strong>{hasSafetyInspectionReport ? "安全报告已归档" : "安全报告缺失"}</strong><span>{hasEmissionsInspectionReport ? "含排放检验报告" : "排放报告未提供（选填）"}</span></div>
